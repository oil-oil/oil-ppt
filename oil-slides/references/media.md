...
运行 `scripts/oil-slides media plan <项目> --write`。程序会输出槽位比例、目标尺寸、frame owner、文件状态与哈希；需要截图适配时还会给出完整 `media frame` 命令。不要先生成任意比例图片再硬塞进页面。
...
获取外部图片时必须回到原始授权页；普通搜索结果图不能直接进入正式预览。渠道的机器可读策略由 `scripts/oil-slides media sources` 输出。
