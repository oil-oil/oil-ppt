...
ANCHOR_SILHOUETTES = {"bleed", "browser", "canvas", "diagram", "metric", "rail", "split", "timeline", "editorial-feature", "case-board", "annotated", "gallery"}
...
def has_media(slide: dict) -> bool:
    if any(str(slide.get(key) or "").strip() for key in MEDIA_KEYS):
        return True
    return slide.get("template") == "sequence-gallery" and any(
        isinstance(step, dict) and str(step.get("image") or "").strip()
...
def audit_outline(data: dict) -> list[dict]:
    ...
    for review in capability_review["slides"]:
        ...
        results.append(issue(
            "warning", "specialized-capability-missed",
            f"Slide {review['id']!r} contains a high-confidence specialized intent but selected {review['selected']!r}.",
            [review["id"]],
            {
                "action": "review the specialized candidates before keeping the generic selection",
    ...
    for segment_index, segment in enumerate(content_segments(slides), start=1):
        ...
        anchors = [slide for slide in segment if slide_quality(slide)["silhouette"] in ANCHOR_SILHOUETTES]
