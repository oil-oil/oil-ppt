...
- `media_surface`：仅 `split-visual` / `editorial-feature` 使用；概念插画可设 `page-blend` 直接融入页面，普通真实证据使用 `component`；素材已经自带完整界面、设备或文档边界时声明 `media_frame: self-framed`，程序会自动无框融页。
...
- 通用灰色圆角容器不是默认图片风格：封面主视觉直接铺满开放版位；概念插画优先融页；只有真实证据本身没有边界、且需要表达窗口、设备或文档边界时才由组件补一层语义外框。素材已有边界时不得再套灰框。
