...
生成正式预览前，根据主题、受众、内容类型、用户语气和已提供的品牌信息，主动选择合法的配色、字体、圆角、页面密度、模板与组件倾向；不要把内部预设或视觉参数交给用户逐项选择。正式预览默认打开可编辑页面，页面顶部只读展示当前视觉摘要，用户点击页面校对文字。用户说“更温暖一点”“不要这么圆”“换成品牌蓝”或“更像杂志”时，将自然语言统一翻译成合法视觉参数，应用到整套演示并重新检查全部页面，不要求用户说出 token 名称。文字编辑共享草稿、撤销、重做和还原；设计调整仍保留为 Agent 的内部能力。完成后程序写回结构化内容并重新生成预览。不要直接修改预览或最终 HTML。生成后停止等待用户确认；确认后仍只执行状态机返回的命令，`build` 会自行完成脚手架、素材内联和浏览器验证。
