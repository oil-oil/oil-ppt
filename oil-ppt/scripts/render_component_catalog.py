...
def render_catalog(output: Path) -> int:
    ...
    *{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font-family:"PingFang SC","Noto Sans SC",system-ui,sans-serif}}button{{font:inherit}}
    ...
    .hero::after{{content:"";position:absolute;right:-132px;top:-132px;width:420px;height:420px;border:48px solid #f1f1ef;border-radius:50%;clip-path:inset(50% 50% 0 0);pointer-events:none}}
    ...
    .primitives{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:30px}}.primitive{{position:relative;min-height:180px;padding:28px;border-radius:28px;background:#f2f2ef;overflow:visible}}.primitive.accent{{background:var(--accent-soft)}}.primitive.ink{{background:var(--ink);color:#fff}}.primitive b{{position:relative;z-index:1;font-size:24px}}.primitive span{{position:absolute;z-index:1;left:28px;bottom:26px;color:var(--muted)}}.primitive.ink span{{color:#bbb}}
    .ring::after{{content:"";position:absolute;right:-72px;bottom:-72px;width:200px;height:200px;border:28px solid rgba(45,45,43,.07);border-radius:50%;clip-path:inset(0 50% 50% 0);pointer-events:none}}.dots::after{{content:"";position:absolute;right:20px;bottom:18px;width:126px;height:92px;background:radial-gradient(circle,rgba(45,45,43,.12) 2px,transparent 2.5px);background-size:18px 18px}}
