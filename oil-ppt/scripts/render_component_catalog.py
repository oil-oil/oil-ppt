...
def render_catalog(output: Path) -> int:
    ...
    *{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font-family:"PingFang SC","Noto Sans SC",system-ui,sans-serif}}button{{font:inherit}}
    ...
    .hero{{position:relative;min-height:520px;padding:76px 80px;border:1px solid var(--line);border-radius:44px;background:var(--surface);overflow:hidden}}
    .hero::after{{content:"";position:absolute;right:-170px;top:-180px;width:520px;height:520px;border:54px solid #f1f1ef;border-radius:50%}}
    ...
    .system{{margin-top:22px;padding:42px 48px;border:1px solid var(--line);border-radius:36px;background:var(--surface)}}.system h2{{margin:0;font-size:38px}}.system p{{max-width:990px;color:var(--muted);font-size:20px;line-height:1.65}}
    .primitives{{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:30px}}.primitive{{position:relative;min-height:180px;padding:28px;border-radius:28px;background:#f2f2ef;overflow:hidden}}.primitive.accent{{background:var(--accent-soft)}}.primitive.ink{{background:var(--ink);color:#fff}}.primitive b{{position:relative;z-index:1;font-size:24px}}.primitive span{{position:absolute;left:28px;bottom:26px;color:var(--muted)}}.primitive.ink span{{color:#bbb}}
    .ring::after{{content:"";position:absolute;right:-66px;bottom:-76px;width:170px;height:170px;border:24px solid rgba(45,45,43,.07);border-radius:50%}}.dots::after{{content:"";position:absolute;right:20px;bottom:18px;width:126px;height:92px;background:radial-gradient(circle,rgba(45,45,43,.12) 2px,transparent 2.5px);background-size:18px 18px}}
    .filters{{position:sticky;top:12px;z-index:20;display:flex;gap:10px;flex-wrap:wrap;margin:22px 0;padding:14px;border:1px solid var(--line);border-radius:26px;background:rgba(255,255,255,.9);backdrop-filter:blur(16px)}}.filters button{{padding:12px 18px;border:0;border-radius:16px;background:#efefec;color:var(--ink);cursor:pointer}}.filters button.is-active{{background:var(--ink);color:#fff}}
    .family-section{{margin-top:26px;padding:34px;border:1px solid var(--line);border-radius:36px;background:var(--surface)}}.section-head{{display:flex;justify-content:space-between;align-items:center;margin:0 0 24px}}.section-head span{{font-size:34px;font-weight:800}}.section-head b{{color:var(--muted);font-size:17px}}
    ...
    .preview{{position:relative;aspect-ratio:16/9;border-radius:22px;background:#fff;overflow:hidden;box-shadow:0 14px 34px rgba(45,45,43,.08)}}iframe{{position:absolute;inset:0;width:100%;height:100%;border:0;pointer-events:none}}.component-card footer{{display:flex;justify-content:space-between;padding:15px 6px 3px;color:var(--muted)}}code{{font-size:13px}}
    ...
    @media(max-width:980px){{.page{{width:calc(100% - 20px);margin-top:10px}}.hero{{padding:48px 32px;min-height:auto}}h1{{font-size:52px}}.primitives,.component-grid{{grid-template-columns:1fr}}.stats{{flex-wrap:wrap}}}}
      ...
      <section class="system"><h2>先看程序自动承担的视觉原语</h2><p>模型只选择内容关系。容器底色、低对比几何、点阵、圆角与图标框架由模板和 runtime 统一生成，因此不会因为模型能力不同而变成彩色卡片拼盘。</p><div class="primitives"><article class="primitive accent"><b>主题色浅层</b><span>重点容器</span></article><article class="primitive ring"><b>中性圆环</b><span>低对比几何</span></article><article class="primitive dots"><b>局部点阵</b><span>只落一个重点</span></article><article class="primitive ink"><b>深色锚点</b><span>每页最多一个</span></article></div></section>
