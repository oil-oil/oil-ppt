...
from component_contracts import effective_frame_owner, effective_media_fit, effective_media_surface
...
MEDIA_SLOTS = {
    ...
    "browser-showcase": {"ratio": "4:3", "target": [1440, 1080], "role": "product-screenshot", "fit": "contain", "fidelity": "strict"},
    ...
    "photo-split": {"ratio": "1:1", "target": [1200, 1200], "role": "supporting-photo", "fit": "cover", "fidelity": "contextual"},
    "split-visual": {"ratio": "1:1", "target": [1200, 1200], "role": "main-evidence", "fit": "contain", "fidelity": "strict"},
    "editorial-feature": {"ratio": "1:1", "target": [1200, 1200], "role": "main-evidence", "fit": "contain", "fidelity": "strict"},
...
MEDIA_VARIANT_SLOTS = {
    ("split-visual", "copy-dominant"): {"ratio": "4:5", "target": [1200, 1500]},
}
...
def _slot_for(slide: dict) -> dict | None:
    ...
    if template == "case-study-board" and slide.get("variant") != "evidence":
        return None
    slot = MEDIA_SLOTS.get(template)
    if slot is None:
        return None
    return {**slot, **MEDIA_VARIANT_SLOTS.get((template, str(slide.get("variant") or "")), {})}
...
def build_media_plan(data: dict, project: Path, *, cli: str = "scripts/oil-ppt") -> dict:
    ...
    for index, slide in enumerate(data.get("slides") or [], start=1):
        ...
        for media_index, binding in enumerate(bindings, start=1):
            ...
            media_surface = effective_media_surface(slide)
            fit = effective_media_fit(slide, fallback=str(slot["fit"]), fidelity=fidelity)
            frame_owner = effective_frame_owner(slide)
            ...
            if value and media_surface != "page-blend" and frame_owner == "none" and frame_role and media_frame != "self-framed" and can_frame:
                frame_argv = [
            ...
            if media_surface == "page-blend":
                adaptation_reason = "page-blend media stays frameless and uses page-matched or transparent artwork directly"
            elif media_frame == "self-framed":
